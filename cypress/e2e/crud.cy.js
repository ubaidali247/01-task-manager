...
describe('CRUD Operations - 01 Task Manager', () => {
  ...
  it('shows seeded items on tasks list', () => {
    cy.get('#tasks-list .item-card').should('have.length.gte', 1);
  });

  it('creates a new item successfully', () => {
    cy.get('#btn-add-new').click();
    cy.get('#field-title').type('Test Task Title');
    cy.get('#field-description, #field-content, #field-review, #field-notes').first().type('Test description for new item');
    cy.get('#btn-submit').click();
    cy.get('#page-tasks').should('not.have.class', 'hidden');
    cy.get('#tasks-list').should('contain', 'Test Task Title');
  });
  ...
  it('shows new item in the list after creation', () => {
    ...
    cy.get('#btn-submit').click();
    cy.get('#tasks-list').should('contain', title);
  });

  it('item card has view button', () => {
    cy.get('#tasks-list .item-card').first().find('.btn-view').should('be.visible');
  });

  it('item card has edit button', () => {
    cy.get('#tasks-list .item-card').first().find('.btn-edit-card').should('be.visible');
  });

  it('item card has delete button', () => {
    cy.get('#tasks-list .item-card').first().find('.btn-delete-card').should('be.visible');
  });

  it('view button shows detail page', () => {
    cy.get('#tasks-list .item-card').first().find('.btn-view').click();
  ...
  it('edit button from list opens edit form', () => {
    cy.get('#tasks-list .item-card').first().find('.btn-edit-card').click();
  ...
  it('can update an existing item', () => {
    cy.get('#tasks-list .item-card').first().find('.btn-edit-card').click();
    cy.get('#field-title').clear().type('Updated Task Title');
    cy.get('#btn-submit').click();
    cy.get('#tasks-list').should('contain', 'Updated Task Title');
  });

  it('delete shows confirmation dialog', () => {
    cy.on('window:confirm', () => false);
    cy.get('#tasks-list .item-card').first().find('.btn-delete-card').click();
  });

  it('confirming delete removes item from list', () => {
    cy.get('#tasks-list .item-card').then($cards => {
      const initialCount = $cards.length;
      cy.on('window:confirm', () => true);
      cy.get('#tasks-list .item-card').first().find('.btn-delete-card').click();
      cy.get('#tasks-list .item-card').should('have.length', initialCount - 1);
    });
  });

  it('api returns items as array', () => {
    cy.request('/api/tasks').its('body').should('be.an', 'array');
  });
